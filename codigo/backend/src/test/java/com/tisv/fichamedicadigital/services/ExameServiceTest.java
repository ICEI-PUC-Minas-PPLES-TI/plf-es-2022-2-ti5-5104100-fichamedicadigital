...
public class ExameServiceTest {
	...
	void setUp() throws Exception {
		...
		idDependente = 3L;
		exame = new Exame();
		page = new PageImpl<>(List.of(exame));

		Mockito.when(repository.findById(idNaoExistente)).thenReturn(Optional.empty());
		Mockito.when(repository.findById(idExistente)).thenReturn(Optional.of(exame));
		Mockito.when(repository.save(ArgumentMatchers.any())).thenReturn(exame);
	...
	public void findAllPagedDeveRetornarUmaPagina() {
		...
		Page<Exame> result = service.findAllPaged(pageable);

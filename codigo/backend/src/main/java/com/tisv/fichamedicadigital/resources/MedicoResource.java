...
public class MedicoResource {
	 ...
	 */
	@GetMapping(value = "/{id}")
	public ResponseEntity<Medico> findBydId(@PathVariable Long id) {
		Medico medico = service.findByUsuario(id);
		return ResponseEntity.ok().body(medico);
	}

	/***
	 * Método para criar um médico. A partir do Id do usuário. Ele irá criar um médico a partir desse usuário e irá retirar
	 * o status de paciente
	 * @param Id do usuário
	 * @return Retorna no médico criado
	 */
	@PostMapping
	public ResponseEntity<Medico> insert(@RequestBody UsuarioDTO id) {
